...
local function spawnAI(sim)
    ...
    sim.Crash = function(inst)
        local a = inst.components.crashGame.yay
    end
...
local function RoGOnUpdate(self, dt)
    ...
    self.OnUpdate = function(self, dt)
        ...
        if self.dest then
            ...
            if dsq <= math.max(run_dist * run_dist, self.arrive_dist * self.arrive_dist) then
                self.inst:PushEvent("onreachdestination", {
                    target = self.dest.inst,
                    pos = Point(destpos_x, destpos_y, destpos_z)
                })
                    ...
                    if pathstatus ~= STATUS_CALCULATING then
                        ...
                        if pathstatus == STATUS_FOUNDPATH then
                            ...
                            if foundpath then
                                ...
                                self.inst:PushEvent("noPathFound", {
                                    inst = self.inst,
                                    target = self.dest.inst,
                                    pos = Point(destpos_x, destpos_y, destpos_z)
                                })
        ...
        local should_locomote = (not is_moving ~= not self.wantstomoveforward) or
                                    (is_moving and (not is_running ~= not self.wantstorun)) -- 'not' is being used on this line as a cast-to-boolean operator
