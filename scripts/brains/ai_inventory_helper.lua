...
function ShouldGoInBackpack(item)
    ...
    if item.components.armor or item.components.weapon then
        return false
    end

    -- Don't put essentials in there
    if notInBackpack[item.prefab] then
        return false
    end
...
function CanFitInStack(inst, item)
    ...
    local inInv = inst.components.inventory:FindItems(function(i)
        return item.prefab == i.prefab
    end)
...
function TransferItemTo(item, fromInst, toInst, fullStack)
    ...
    if not item or not fromInst or not toInst then
        return false
    end

    local fromContainer = fromInst.components.inventory and fromInst.components.inventory or
                              fromInst.components.container
    ...
    if toContainer:IsFull() then
        local itemInDest = toContainer:FindItems(function(i)
            return i.prefab == item.prefab
        end)
