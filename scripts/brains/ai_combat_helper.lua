...
function GoForTheEyes(inst)
    ...
    local closestHostile = FindEntity(inst, 20, function(guy)
        return guy:HasTag("hostile") and inst.components.combat:CanTarget(guy)
    end)
    ...
    local allWeaponsInInventory = inst.components.inventory:FindItems(function(item)
        return item.components.weapon and item.components.equippable and item.components.weapon.damage > 0
    end)
    ...
    if highestDamageWeapon == nil or (highestDamageWeapon and highestDamageWeapon.components.weapon.damage < 34) then
        ...
        if canBuildSpear then
            ...
            inst.brain:SetSomethingToBuild("spear", nil, function()
                inst.waitingForBuild = nil
            end, function()
                inst.waitingForBuild = nil
            end)
    ...
    for k, v in pairs(allHostiles) do
        ...
        totalWeaponSwings = totalWeaponSwings +
                                math.ceil(
                v.components.health.currenthealth / highestDamageWeapon.components.weapon.damage)
    end
...
function ShouldRunAway(guy)
    ...
    if guy:HasTag("worker") and GetSeasonManager() and GetSeasonManager().IsSpring ~= nil and
        GetSeasonManager():IsSpring() then
        return true
    end
    ...
    return guy:HasTag("WORM_DANGER") or guy:HasTag("guard") or guy:HasTag("hostile") or guy:HasTag("scarytoprey") or
               guy:HasTag("frog") or guy:HasTag("mosquito") or guy:HasTag("merm") or guy:HasTag("tallbird")
