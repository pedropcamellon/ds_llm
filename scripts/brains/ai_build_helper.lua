...
function CanPrototypeRecipe(recipetree, buildertree)
    for k, v in pairs(recipetree) do
        if buildertree[tostring(k)] and recipetree[tostring(k)] and recipetree[tostring(k)] > buildertree[tostring(k)] then
            return false
        end
    end
    return true
    ...
    for k, v in pairs(player.itemsNeeded) do
        ...
        if v.level > 0 and v.toMake then
            ...
            while v.toMakeNum > 0 do
                ...
                local action = BufferedAction(player, nil, ACTIONS.BUILD, nil,
                    Point(player.Transform:GetWorldPosition()), v.toMake, 1)
                if onFail then
