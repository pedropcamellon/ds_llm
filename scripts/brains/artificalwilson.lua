...
local function CanIBuildThis(player, thingToBuild, numToBuild, recursive)
    ...
    for ik, iv in pairs(recipe.ingredients) do
        ...
        if numHas < totalAmountNeeded then
            ...
            if numHas > 0 then
                table.insert(player.itemsNeeded, 1, {
                    prefab = iv.type,
                    amount = numHas,
                    level = 0
                })
            ...
            print(
                "Adding " .. tostring(totalAmountNeeded) .. " of " .. iv.type .. " at level " .. tostring(recursive) ..
                    " to the itemsNeeded list")
            table.insert(player.itemsNeeded, 1, {
                prefab = iv.type,
                amount = totalAmountNeeded,
                level = recursive,
                toMake = thingToBuild,
                toMakeNum = numToBuild
            })
        end
    end
...
function ArtificalBrain:HostileMobNearInst(inst)
    ...
    if pos then
        return FindEntity(inst, RUN_AWAY_SEE_DIST, function(guy)
            return ShouldRunAway(guy)
        end) ~= nil
    end
    return false
end
...
local function OnPathFinder(self, data)
    ...
    if data then
        ...
        if data.target then
            ...
            self.components.prioritizer:AddToIgnoreList(data.target.entity:GetGUID(),
                Vector3(self.Transform:GetWorldPosition()))
        end
    end
...
local function FindValidHome(inst)
    ...
    if not HasValidHome(inst) and inst.components.homeseeker then
        ...
        local scienceMachine = FindEntity(inst, 150, function(item)
            return item.prefab and item.prefab == "researchlab"
        end)
...
local function FindSomewhereNewToGo(inst)
    ...
    local wormhole = FindEntity(inst, 200, function(thing)
        return thing.prefab and thing.prefab == "wormhole"
    end)
    if wormhole then
...
function ArtificalBrain:OnStart()
    ...
    if not HasValidHome(self.inst) then
        local scienceMachine = FindEntity(self.inst, 150, function(item)
            return item.prefab and item.prefab == "researchlab"
        end)
        if scienceMachine then
            print("Found our home!")
    ...
    -- Register the brain with the LLM action executor so it can invoke actions
    if self.inst.components.llm_action_executor then
        self.inst.components.llm_action_executor:SetBrain(self)
    end

    -- Things to do during the day
    local day = WhileNode(function()
        return clock and clock:IsDay()
    end, "IsDay", PriorityNode { -- Eat something if hunger gets below .5
    ManageHunger(self.inst, .5), -- If there's a touchstone nearby, activate it
    IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_lookforTouchstone", FindAndActivate(self.inst, 25, "resurrectionstone")),

    -- Find a good place to call home
    IfNode(function()
        return not HasValidHome(self.inst)
    end, "no home", DoAction(self.inst, function()
        return FindValidHome(self.inst)
    end, "looking for home", true)), -- Collect stuff
    SelectorNode {IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goPickup", FindResourceOnGround(self.inst, function()
        return self:GetCurrentSearchDistance()
    end)), IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goHarvest", FindResourceToHarvest(self.inst, function()
        return self:GetCurrentSearchDistance()
    end)), IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goChop", FindTreeOrRock(self.inst, function()
        return self:GetCurrentSearchDistance()
    end, ACTIONS.CHOP)), IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goMine", FindTreeOrRock(self.inst, function()
        return self:GetCurrentSearchDistance()
    end, ACTIONS.MINE)), -- Should maybe stick around and wait for this thing to finish burning...he just
    -- kind of runs away...
    IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goBurn", FindThingToBurn(self.inst, function()
        return self:GetCurrentSearchDistance()
    end)), -- Finally, if none of those succeed, increase the search distance for
    -- the next loop.
    -- Want this to fail always so we don't increase to max.
    IfNode(function()
        return not IsBusy(self.inst)
    end, "nothing_to_do", NotDecorator(ActionNode(function()
        return self:IncreaseSearchDistance()
    end)))}, -- TODO: Need a good wander function for when searchdistance is at max.
    IfNode(function()
        return not IsBusy(self.inst) and CurrentSearchDistance == MAX_SEARCH_DISTANCE
    end, "maxSearchDistance", FindAndActivate(self.inst, 500, "wormhole"))}, .25)

    -- Do this stuff the first half of duck (or all of dusk if we don't have a home yet)
    local dusk = WhileNode(function()
        return clock and clock:IsDusk() and (not MidwayThroughDusk() or not HasValidHome(self.inst))
    end, "IsDusk", PriorityNode { -- Make sure we eat. During the day, only make sure to stay above 50% hunger.
    ManageHunger(self.inst, .5), -- Find a good place to call home
    IfNode(function()
        return not HasValidHome(self.inst)
    end, "no home", DoAction(self.inst, function()
        return FindValidHome(self.inst)
    end, "looking for home", true)), SelectorNode {IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goPickup", FindResourceOnGround(self.inst, function()
        return self:GetCurrentSearchDistance()
    end)), IfNode(function()
        ...
        return self:GetCurrentSearchDistance()
    end, ACTIONS.MINE)), IfNode(function()
        return not IsBusy(self.inst)
    end, "nothing_to_do", NotDecorator(ActionNode(function()
        return self:IncreaseSearchDistance()
    end)))}, -- This is super hacky.
    IfNode(function()
        return not IsBusy(self.inst) and CurrentSearchDistance == MAX_SEARCH_DISTANCE
    end, "maxSearchDistance", DoAction(self.inst, function()
        return FindSomewhereNewToGo(self.inst)
    end, "lookingForSomewhere", true)) -- No plan...just walking around
    -- Wander(self.inst, nil, 20),
    }, .2)

    -- Behave slightly different half way through dusk
    local dusk2 = WhileNode(function()
        return clock and clock:IsDusk() and MidwayThroughDusk() and HasValidHome(self.inst)
    end, "IsDusk2",
        PriorityNode { -- IfNode( function() return not IsBusy(self.inst) and  self.inst.components.hunger:GetPercent() < .5 end, "notBusy_hungry",
        --	DoAction(self.inst, function() return HaveASnack(self.inst) end, "eating", true )),
        ManageHunger(self.inst, .5), IfNode(function()
            return HasValidHome(self.inst)
        end, "try to go home", DoAction(self.inst, function()
            return GoHomeAction(self.inst)
        end, "go home", true)) -- If we don't have a home...just
    ...
    local night = WhileNode(function()
        return clock and clock:IsNight()
    end, "IsNight", PriorityNode { -- TODO: If we aren't home but we have a home, make a torch and keep running!
    -- Make sure there's light!
    -- Moved this higher in the brain
    -- MaintainLightSource(self.inst, 30),
    CookFood(self.inst, 10), -- IfNode( function() return IsNearCookingSource(self.inst) end, "let's cook",
    ...
    --	DoAction(self.inst, function() return HaveASnack(self.inst) end, "eating", true )),
    ManageHunger(self.inst, .9), -- Collect stuff
    SelectorNode {IfNode(function()
        return not IsBusy(self.inst)
    end, "notBusy_goPickup", FindResourceOnGround(self.inst, function()
        return 5
    ...
    local nonAIMode = PriorityNode({WhileNode(function()
        return TheInput:IsControlPressed(CONTROL_PRIMARY)
    end, "Hold LMB", ChaseAndAttack(self.inst, MAX_CHASE_TIME)), ChaseAndAttack(self.inst, MAX_CHASE_TIME, nil, 1)}, 0)

    local root = PriorityNode(
        { -- If any brain function decides necessar, it can add an IsStuck tag to wilson. This will cause the brain to reset.
        IfNode(function()
            return self.inst:HasTag("IsStuck")
        end, "stuck", DoAction(self.inst, function()
            print("Trying to fix this...")
            return FixStuckWilson(self.inst)
        end, "alive3", true)), -- If we ever get something in our overflow slot in the inventory, drop it.
        -- This will happen occationally if we try to build something we can't hold...etc.
        IfNode(function()
            return self.inst.components.inventory.activeitem ~= nil
        end, "drop_activeItem", DoAction(self.inst, function()
            local inv = self.inst.components.inventory
            local item = inv:GetActiveItem()
            if item then
                inv:DropItem(item, true, true)
            end
        end, "drop", true)), -- Quit standing in the fire, idiot
        -- WhileNode(function() return self.inst.components.health.takingfiredamage end, "OnFire", Panic(self.inst) ),
        DontBeOnFire(self.inst), --
        WhileNode(function()
            return clock and clock:IsNight()
        end, "StayInTheLight", MaintainLightSource(self.inst, 30)),
        ...
        WhileNode(function()
            return not clock:IsNight() and GoForTheEyes(self.inst)
        end, "GoForTheEyes", ChaseAndAttack(self.inst, 10, 30)),
        -- DoAction(self.inst, function() return GoForTheEyes(self.inst) end, "GoForTheEyes", true),

        -- Always run away from these things
        -- RunAway(self.inst, function(guy) return ShouldRunAwayLoud(guy) end, RUN_AWAY_SEE_DIST, RUN_AWAY_STOP_DIST, function(target) return true end),
        -- Passed in function should return an entity.
        -- RunAway(self.inst, function(myself) return FindThingToRunFrom(myself, RUN_AWAY_SEE_DIST) end, RUN_AWAY_SEE_DIST, RUN_AWAY_STOP_DIST),
        RunAway(self.inst, function(myself)
            return FindEntity(myself, RUN_AWAY_SEE_DIST, function(guy)
                return ShouldRunAway(guy)
            end, nil, {"INLIMBO", "notarget"})
        end, RUN_AWAY_SEE_DIST, RUN_AWAY_STOP_DIST), -- SelfPreservation(self.inst,
        -- Manage rain by making/equipping straw hat
        ManageClothes(self.inst), -- Try to stay healthy
        IfNode(function()
            return not IsBusy(self.inst)
        end, "notBusy_heal", ManageHealth(self.inst, .75)), -- Try to stay sane
        ...
        IfNode(function()
            return not IsBusy(self.inst)
        end, "notBusy_sanity", ManageSanity(self.inst, function()
            return .9, .75
        end)), -- Hunger is managed during the days/nights
        ...
        DoScience(self.inst, function()
            return self:GetSomethingToBuild()
        end), -- Only do these things not very often
        PriorityNode({MasterChef(self.inst), ManageInventory(self.inst)}, 2.5), day, dusk, dusk2, night}, .25)
