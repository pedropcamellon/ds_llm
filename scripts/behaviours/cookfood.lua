...
function CookFood:Visit()
    ...
    if self.status == READY then
        ...
        local cookableFood = self.inst.components.inventory:FindItem(function(item)
            ...
            if not item.components.cookable then
                return false
            end

            local has, numfound = self.inst.components.inventory:Has(item.prefab, 1)
            local theProduct = self.inst.components.inventory:FindItem(function(result)
                return result.prefab == item.components.cookable.product
            end)
            local canFillStack = false
            if theProduct then
                canFillStack = not self.inst.components.inventory:Has(item.components.cookable.product,
                    theProduct.components.stackable.maxsize)
            end
