...
function FindResourceOnGround:Visit()
    ...
    if self.status == READY then
        ...
        if not IsWearingBackpack(self.inst) then
            local backpack = FindEntity(self.inst, 30, function(item)
                return IsItemBackpack(item)
            end)
            if backpack then
        ...
        local target = FindEntity(self.inst, self.distance(), function(item)
            ...
            if self.inst.components.prioritizer:OnIgnoreList(item.prefab) or
                self.inst.components.prioritizer:OnIgnoreList(item.entity:GetGUID()) then
                return false
            end

            -- Ignore backpack (covered above)
            if IsItemBackpack(item) then
                return false
            end

            -- Ignore these dang trinkets
            if item.prefab and string.find(item.prefab, "trinket") then
                return false
            end
            -- We won't need these thing either.
            if item.prefab and string.find(item.prefab, "teleportato") then
                return false
            end
            ...
            local haveFullStack, num = self.inst.components.inventory:Has(item.prefab, item.components.stackable and
            ...
            if item.components.inventoryitem and item.components.inventoryitem.canbepickedup and
                not item.components.inventoryitem:IsHeld() and item:IsOnValidGround() and not item:HasTag("prey") and
