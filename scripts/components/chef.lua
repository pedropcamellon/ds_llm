local Chef = Class(function(self, inst)
    ...
    self.neverMake = {"wetgoop", "fishtacos", "unagi", "jammypreserves", "kabobs", "stuffedeggplant", "ratatouille",
                      "monsterlasagna", "powcake"}
...
function Chef:MakeSomethingGood(cooker, onsuccess, onfail)
    ...
    for prefab, num in pairs(whatImMaking[randCombo].combo.names) do
        ...
        local ing = self.inst.components.inventory:FindItem(function(item)
    ...
    for k, v in pairs(invItems) do
        ...
        self.inst:DoTaskInTime(tdelay, function()
            TransferItemTo(v, self.inst, cooker, false)
        end)
   ...
   --]]
    ...
    action:AddSuccessAction(function()
        if math.random() < .1 then
            self.inst.components.talker:Say("I'm mother fucking Gordon Ramsay")
        end
        if onsuccess then
            onsuccess()
        end
    ...
    self.inst:DoTaskInTime(tdelay + (2 * interval), function()
